...
<<<<<<< HEAD
    ...
    $json = ConvertTo-Json $root -Depth 10 -Compress:$false
    # Add JSONC comment at top for Kilo Code
    return "// MCP server configuration for Kilo Code`n// Generated by 1c-rules installer`n// Enable servers by setting 'enabled': true`n// Servers are disabled by default for security`n`n" + $json
}
